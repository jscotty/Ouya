...
public class DrawLine : MonoBehaviour {
	...
	void UpdateLine()
	{
		...
		else if(lines.Count - 1 == currentLineID) // If that line actually already does exist, update it.
		{
			...
			if (linePoints.Count > 0)
			{
				...
				colliderKeeper.transform.SetParent(newLineHolder.transform);
				BoxCollider bc = colliderKeeper.AddComponent<BoxCollider>();
				bc.material = colliderMaterial;
				colliderKeeper.transform.position = Vector3.Lerp(linePoints[linePoints.Count - 1], linePoints[linePoints.Count - 2], 0.5f);
				colliderKeeper.transform.LookAt(linePoints[linePoints.Count - 1]);
				bc.size = new Vector3(0.25f, 0.25f, Vector3.Distance(linePoints[linePoints.Count - 1], linePoints[linePoints.Count - 2]));
				*/
			}
